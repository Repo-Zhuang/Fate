...
class LogisticParam(BaseParam):
    ...
    def check(self):
        ...
        # 增加对bcp的识别
        if self.encrypt_param.method not in [consts.PAILLIER, consts.BCP, None]:
            raise ValueError(
                "logistic_param's encrypted method support 'Paillier' 'BCP' or None only")
...
class HomoLogisticParam(LogisticParam):
    ...
    """
    # 不使用正则化，优化方式为随机梯度下降    记得修改method

    def __init__(self, penalty=None,
                 tol=1e-4, alpha=1.0, optimizer='sgd',
                 batch_size=-1, learning_rate=0.01, init_param=InitParam(),
                 max_iter=100, early_stop='diff',
                 encrypt_param=EncryptParam(method=consts.BCP), re_encrypt_batches=2,
                 predict_param=PredictParam(), cv_param=CrossValidationParam(),
                 decay=1, decay_sqrt=True,
    ...
    def check(self):
        ...
        if self.encrypt_param.method in [consts.PAILLIER, consts.BCP]:
            if self.optimizer != 'sgd':
                raise ValueError(f"{self.encrypt_param.method} encryption mode supports 'sgd' optimizer method only.")

            if self.penalty == consts.L1_PENALTY:
                raise ValueError(f"{self.encrypt_param.method} encryption mode supports 'L2' penalty or None only.")
