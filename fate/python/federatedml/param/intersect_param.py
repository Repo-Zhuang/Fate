...
class VOLEParam(BaseParam):
    """
    Specify parameters for VOLE intersect method
    ...
    seed_str: str
        string for Random Oracle of vole, default ''
        Model will choose random string if not specified.

    stat_sec_param: povitive int
        the statistic security parameter of vole intersect algorithm will be 2^(- stat_sec_param)
        support 40, 50, 60, 80, default 40

    ip_addr: str
        the ip address and socket of the TCP/IP connection server.
        in form of "ip4:socket", default "localhost:12301"

    is_server: bool
        whether is the TCP connection server, one role be true and the other is false.

    is_malicious: bool
        whether runs the protocol in malicious setting, default False

    threat_num: prositive int
        threat number, support 1, 2 and 4, default 1
    """

    def __init__(self, salt='', seed_str='', stat_sec_param=40, ip_addr="localhost:12301",
                 is_server=True, is_malicious=False, threat_num=1):
        super().__init__()
        self.salt = salt
        self.ip_addr = ip_addr
        self.seed_str = seed_str
        self.is_server = is_server
        self.threat_num = threat_num
    ...
    def check(self):
        ...
        self.check_string(self.salt, f"{descr}salt")
        self.check_string(self.seed_str, f"{descr}seed_str")
        self.check_string(self.ip_addr, f"{descr}ip_addr")

        self.check_boolean(self.is_server, f"{descr}is_server")
        self.check_boolean(self.is_malicious, f"{descr}is_malicious")
       
        self.check_valid_value(self.stat_sec_param, f"{descr}stat_sec_param", [40, 50, 60, 80])
        self.check_valid_value(self.threat_num, f"{descr}threat_num", [1, 2, 4])

        LOGGER.debug("Finish VOLEParam parameter check!")
        return True
...
class IntersectParam(BaseParam):
    ...
    intersect_method: str
        it supports 'rsa', 'raw', 'dh' and 'vole', default by 'rsa'
    ...
    vole_params: VOLEParam
        effective for vole method only
    ...
    def __init__(self, intersect_method: str = consts.RSA, random_bit=DEFAULT_RANDOM_BIT, sync_intersect_ids=True,
                 ...
                 raw_params=RAWParam(), rsa_params=RSAParam(), dh_params=DHParam(), vole_params=VOLEParam(),
        ...
        # self.vole_params = copy.deepcopy(vole_params)
    ...
    def check(self):
        ...
        # intersect_method 检查中加入 consts.VOLE
        self.intersect_method = self.check_and_change_lower(self.intersect_method,
                                                            [consts.RSA, consts.RAW, consts.DH, consts.VOLE],
