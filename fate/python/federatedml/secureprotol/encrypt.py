...
from federatedml.secureprotol.fate_bcp import BCP, BCPEncryptedNumber
...
class BCPEncrypt(Encrypt):
    # # 静态属性
    # bcp = BCP()  # 多个实例化之后 主密钥mk已确定
    # mk = bcp.GetMK()
    # param = bcp.GetParam()
    
    def __init__(self, param = None):
        super(BCPEncrypt, self).__init__()
        self.bcp = BCP(param = param)
        # self.mk = self.bcp.GetMK()
        self.param = self.bcp.GetParam()
        
    
    def generate_key(self):
        self.public_key, self.privacy_key = self.bcp.KeyGen()
        return self.public_key, self.privacy_key

    def encrypt(self, pk, value):
        if isinstance(value, FixedPointNumber):
            value = value.decode()
        encoding = FixedPointNumber.encode(value)
        ciphertext = self.bcp.Encrypt(pk, encoding.encoding)
        # pk = int(pk)
        encryptednumber = BCPEncryptedNumber(pk, ciphertext, encoding.exponent)
        
        return encryptednumber
    
    def decrypt(self, sk, encryptednumber):
        if not isinstance(encryptednumber, BCPEncryptedNumber):
            raise TypeError("encrypted_number should be an BCPEncryptedNumber, \
                             not: %s" % type(encryptednumber))
        
        encoded = int(self.bcp.Decrypt(encryptednumber.ciphertext, sk))
        encoded = FixedPointNumber(encoded, encryptednumber.exponent)
        decrypt_value = encoded.decode()
        
        return decrypt_value
    
    def decryptMK(self, mk, encryptednumber):
        if not isinstance(encryptednumber, BCPEncryptedNumber):
            raise TypeError("encrypted_number should be an BCPEncryptedNumber, \
                             not: %s" % type(encryptednumber))
        
        encoded = int(self.bcp.DecryptMK(encryptednumber.ciphertext, mk, encryptednumber.public_key))
    ...
    def add_encryptednumber(self, x, y):
        if x.public_key != y.public_key:
            raise ValueError("add two numbers have different public key!")
        
        def __align_exponent(x, y, N2):
            """return x,y with same exponet
            """
            if x.exponent < y.exponent:
                x = x.increase_exponent_to(y.exponent, N2)
            elif x.exponent > y.exponent:
                y = y.increase_exponent_to(x.exponent, N2)

            return x, y
        # exponents must match, and align.
        N2 = self.param.N2
        
        x, y = __align_exponent(x, y, N2)
        
        encryptednumber = self.bcp.multiply(x.ciphertext, y.ciphertext)
        bcpencryptednumber = BCPEncryptedNumber(x.public_key, encryptednumber, x.exponent)
        
        return bcpencryptednumber
        
    def add_list(self, values, y):
        result = [self.add_encryptednumber(msg, y) for msg in values]
        return result
    
    def encrypt_list(self, pk, values):
        result = [self.encrypt(pk, msg) for msg in values]
        return result    

    def decrypt_list(self, sk, values):
        result = [self.decrypt(sk, msg) for msg in values]
        return result
    
    def decryptMK_list(self, mk, values):
        result = [self.decryptMK(mk, msg) for msg in values]
        return result
