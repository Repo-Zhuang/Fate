...
class HomoLRHost(HomoLRBase):
    ...
    def fit(self, data_instances=None, validate_data=None):
        ...
        # 获取主密钥
        mk = self.cipher_operator.bcp.GetMK()
        param = self.cipher_operator.bcp.GetParam()
        
        print('发送前', param.print_members())
        # 发送公共参数
        param.N2 = int(param.N2)
        param.N = int(param.N)
        param.g = int(param.g)
        param.k = int(param.k)
        print('发送后', param.print_members())
        self.cipher.send_public_param(param= param)
        ...
        while self.n_iter_ < self.max_iter + 1:
            ...
            if ((self.n_iter_ + 1) % self.aggregate_iters == 0) or self.n_iter_ == self.max_iter:
                ...
                print('models_decrypted:', models_decrypted)
                # 聚合
                ...
                print('guest_and_models:', guest_and_models)
                # 发送
