...
# 在类外部定义函数
def sigmoid_map_function(v, coef, intercept):
    return activation.sigmoid(vec_dot(v.features, coef) + intercept)
...
class HomoLRGuest(HomoLRBase):
    ...
    def fit(self, data_instances, validate_data=None):
        ...
        # 接收公共参数
        param = self.cipher.get_public_param()
        param = param[0]
        print('刚刚接收',param.print_members())
        
        
        # 生成bcp加密并发送公钥
        bcp_encrypt = self.cipher.gen_bcp_encrypt_and_send_pk(enable=self.use_encrypt, param= param)
        ...
        while self.n_iter_ < max_iter + 1:
            ...
            if ((self.n_iter_ + 1) % self.aggregate_iters == 0) or self.n_iter_ == max_iter:
                ...
                # -----------
                # 加密
                print('before_aggregate:', model_weights._weights)
                ...
                print('aggregated_weights:', aggregated_weights)
                # ------------
    ...
    def predict(self, data_instances):
        ...
        # 使用外部函数
        pred_prob = data_instances.mapValues(lambda v: sigmoid_map_function(v, coef, intercept))
    ...
    #     # 假设 self.model_weights.coef_ 和 self.model_weights.intercept_ 是 numpy 数组
    #     coef = self.model_weights.coef_
    #     intercept = self.model_weights.intercept_

    #     try:
    #         pickle.dumps(coef)
    #         pickle.dumps(intercept)
    #         print("Coef and intercept are serializable")
