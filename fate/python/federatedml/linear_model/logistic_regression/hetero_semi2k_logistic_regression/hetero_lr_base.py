...
from federatedml.transfer_variable.transfer_class.hetero_semi2klr_transfer_variable import Semi2kLRTransferVariable
...
Semi2k_Machine = '../../../../../../../../../../fate/python/federatedml/secureprotol/semi2k-party.x'
...
class Semi2kLRBase(BaseLinearModel, ABC):
    def __init__(self):
        ...
        self.modelshape_transfer_variable = Semi2kLRTransferVariable()
    ...
    def fit(self, data_instances, validate_data=None):
        ...
        data_instances_id = np.array([i[0] for i in data_instances.collect()])
        data_instances_id = np.array([int(x) for x in data_instances_id])
        LOGGER.debug("data_instances_id:"+str(data_instances_id))
        # 获取排序索引
        sort_indices = np.argsort(data_instances_id)
        ...
        if self.role == consts.GUEST:
            ...
            label_array = label_array[sort_indices]
        ...
        if self.local_party.role == consts.GUEST:
            self.modelshape_transfer_variable.model_shape_guest.remote(model_shape_guest, role="host", idx=0)
            model_shape_host = self.modelshape_transfer_variable.model_shape_host.get(idx=0)
        else:
            model_shape_guest = self.modelshape_transfer_variable.model_shape_guest.get(idx=0)
            self.modelshape_transfer_variable.model_shape_host.remote(model_shape_host, role="guest", idx=0)
