...
class ABYFixedPointTensor(object):
    ...
    address = ABYHeteroSSHELinRParam().address
    port = ABYHeteroSSHELinRParam().port
    ...
    def _vec_share_add(cls, encoded_vector: np.array, role):
        LOGGER.info("_vec_share_add start")
        # 判断是一个一维向量
        ...
        LOGGER.info("1")
        if role == "server":
            LOGGER.info("server cls.address:"+cls.address)
            LOGGER.info("server cls.port:"+str(cls.port))
        
            result_vec, result_type = vector_operator_execute(vector_add_operator_server(), vec, cls.address, cls.port)
        elif role == "client":
            LOGGER.info("client cls.address:"+cls.address)
            LOGGER.info("client cls.port:"+str(cls.port))
        ...
        LOGGER.info("_vec_share_add end")
