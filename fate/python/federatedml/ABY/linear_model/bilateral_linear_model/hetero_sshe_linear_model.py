...
class ABYHeteroSSHEBase(ABYBaseLinearModel, ABC):
    ...
    def aby_fit_single_model(self, data_instances, validate_data=None):
        ...
        sorted_data_instances_numpy = data_instances_numpy[sort_indices]
        LOGGER.debug("sorted_data_instances_numpy:"+str(sorted_data_instances_numpy))
        
        
        features_numpy = np.array([i.features for i in sorted_data_instances_numpy])
        
        if self.role == consts.GUEST:
            features_numpy = np.hstack((features_numpy, np.ones((features_numpy.shape[0], 1))))

            labels_numpy = np.array([i.label for i in sorted_data_instances_numpy]).reshape((-1, 1))
        else:
            labels_numpy = None
        ...
        while self.n_iter_ < self.max_iter:
            ...
            for aby_batch_idx, aby_batch_data in enumerate(aby_encoded_batch_data):
                ...
                if self.role == consts.HOST:
                    ...
                    LOGGER.debug(f"z_guest: {z_guest} type(z_guest): {type(z_guest)} z_guest.shape: {z_guest.shape}")
                    z_host = z_host.get()
                    LOGGER.debug(f"z_host: {z_host} type(z_host): {type(z_host)} z_host.shape: {z_host.shape}")
