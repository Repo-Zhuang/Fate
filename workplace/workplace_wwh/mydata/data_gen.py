...
def generate_unique_data(n) -> pd.DataFrame:
    ...
    y_values = np.random.uniform(low=0.0, high=1000.0, size=n)
    x_values = np.random.uniform(low=0.0, high=1000.0, size=n)
    
    # df = pd.DataFrame({'id': ids, 'y': y_values, 'x1': x_values, 'x2': x_values, 'x3': x_values, 
    #                     'x4': x_values, 'x5': x_values, 'x6': x_values, 'x7': x_values, 
    #                     'x8': x_values, 'x9': x_values, 'x10': x_values})
                        
    df = pd.DataFrame({'id': ids, 'y': y_values, 'x1': x_values})
...
# save_table(data_host, "intersect_host_100000_10.csv")
...
save_table(data_guest, "intersect_guest_100000_1.csv")